...
function isInsideBackgroundOnlyFunction(node) {
  ...
  while (null !== current) {
    ...
    if (
      ...
      'arrow_function' === kind ||
      'function_expression' === kind
    ) {
      const body = current
        .children()
        .find((c) => 'statement_block' === c.kind());
      if (body) {
        const firstStatement = body
          .children()
          .find((c) => 'expression_statement' === c.kind());
        if (firstStatement) {
...
function findEventHandlerFunctions(root) {
  ...
  for (const attr of jsxAttributes) {
    const nameNode = attr
      .children()
      .find((c) => 'property_identifier' === c.kind());
    ...
    const valueNode = attr.children().find((c) => {
      ...
      return (
        'jsx_expression' === kind ||
        'string' === kind ||
        'string_fragment' === kind
      );
    });
    if (valueNode)
      if ('jsx_expression' === valueNode.kind()) {
        const inner = valueNode.child(1);
        if (inner && 'identifier' === inner.kind())
...
export { analyzeBackgroundOnlyUsage, rules, runSkill };
