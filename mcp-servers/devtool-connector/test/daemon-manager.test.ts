...
test("resolveDaemonEntryPath respects package imports in a built package", async (t) => {
  ...
  await fs.writeFile(path.join(tempDir, "dist", "daemon", "entry.js"), "export {};\n");
  await fs.writeFile(path.join(tempDir, "src", "daemon", "manager.js"), "export {};\n");
  const expectedEntryPath = await fs.realpath(path.join(tempDir, "dist", "daemon", "entry.js"));

  assert.equal(
    resolveDaemonEntryPath(pathToFileURL(path.join(tempDir, "src", "daemon", "manager.js")).href),
    expectedEntryPath,
  );
});
