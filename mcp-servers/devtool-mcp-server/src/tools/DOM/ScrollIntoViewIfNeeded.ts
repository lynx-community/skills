...
export const ScrollIntoViewIfNeeded = /*#__PURE__*/ defineTool({
  ...
  async handler({ params: { clientId, sessionId, nodeId, rect } }, response, context) {
    const connector = context.connector();

    const result = await connector.sendCDPMessage(clientId, sessionId, "DOM.scrollIntoViewIfNeeded", {
      nodeId,
      rect,
    });
