...
export const GetStyleSheetText = /*#__PURE__*/ defineTool({
  ...
  async handler({ params: { clientId, sessionId, styleSheetId } }, response, context) {
    const connector = context.connector();

    // https://chromedevtools.github.io/devtools-protocol/tot/CSS/#method-getStyleSheetText
    const result = await connector.sendCDPMessage(clientId, sessionId, "CSS.getStyleSheetText", {
      styleSheetId,
    });
