...
export class StaticServer {
  ...
  #serveInspectorWrapper(res: http.ServerResponse): void {
    ...
    const filePath = candidates
      ...
      .find((f) => fs.existsSync(f)) ?? path.join(primary, "inspector-wrapper.html");
  ...
  async #serveFrontendRes(req: http.IncomingMessage, res: http.ServerResponse, pathname: string): Promise<void> {
    try {
