...
export type CDPRequestMessage<T = unknown> = { method: string; params?: T | undefined };
...
export type CDPResponseMessage = { id: number } & ({ result: unknown } | { error: { code: number; message: string } });
...
export type ListSessionRequest = CustomizedEvent<"ListSession", Record<string, never>>;
...
export type HeadlessPrepareRequest = CustomizedEvent<"HeadlessPrepare", Record<string, never>>;
export type HeadlessPrepareResponse = CustomizedEvent<"HeadlessPrepare", HeadlessPrepareState>;
...
export type XdbJsbRequest = CustomizedEvent<"xdb_jsb", {
  client_id: number;
  session_id: number;
  message: { type: string };
}>;

export type XdbJsbResponse = CustomizedEvent<"xdb_jsb", {
  client_id: number;
  session_id: number;
  /** JSON string */
  message: string;
...
export type XdbGlobalPropsRequest = CustomizedEvent<"xdb_globalprops", {
  client_id: number;
  session_id: number;
  message: { type: string; timestamp: string };
}>;

export type XdbGlobalPropsResponse = CustomizedEvent<"xdb_globalprops", {
  client_id: number;
  session_id: number;
  /** JSON string */
  message: string;
...
export function isInitializeResponse(response: Response): response is InitializeResponse {
  return response.event === "Register";
}

export function isHeadlessPrepareResponse(response: Response): response is HeadlessPrepareResponse {
  return response.event === "Customized" && response.data.type === "HeadlessPrepare";
}

export function isListSessionResponse(response: Response): response is ListSessionResponse {
  return response.event === "Customized" && response.data.type === "SessionList";
}

export function isGetGlobalSwitchResponse(response: Response): response is GetGlobalSwitchResponse {
  return response.event === "Customized" && response.data.type === "GetGlobalSwitch";
}

export function isSetGlobalSwitchResponse(response: Response): response is SetGlobalSwitchResponse {
  return response.event === "Customized" && response.data.type === "SetGlobalSwitch";
}

export function isCustomizedResponseWithType<T extends keyof CustomizedResponseMap>(
  response: Response,
  type: T,
