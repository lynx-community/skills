...
function createFakeTransport(): {
  ...
  const { readable: deviceReadable, writable: deviceSideWritable } = new TransformStream<unknown>();
...
describe("DeviceConnection", () => {
  test("connect() establishes the connection and exposes key/deviceId/port", async (t) => {
    ...
    const conn = new DeviceConnection(transport, { deviceId: "fake-device", port: 8901 });
    await conn.connect();
  ...
  test("send() forwards messages to the underlying transport", async (t) => {
    const { transport, deviceMessages, closeConnection } = createFakeTransport();
    t.after(() => closeConnection());
  ...
  test("broadcasts device messages to all subscribers", async (t) => {
    ...
    conn.addSubscriber({ id: 1, send: (msg) => receivedA.push(msg), close() {} });
    conn.addSubscriber({ id: 2, send: (msg) => receivedB.push(msg), close() {} });
    ...
    assert.deepStrictEqual(receivedA[0], { event: "Customized", data: { type: "CDP" } });
    assert.deepStrictEqual(receivedB[0], { event: "Customized", data: { type: "CDP" } });
  });
  ...
  test("captures appInfo from Register response and does not broadcast it", async (t) => {
    ...
    conn.addSubscriber({ id: 1, send: (msg) => received.push(msg), close() {} });
