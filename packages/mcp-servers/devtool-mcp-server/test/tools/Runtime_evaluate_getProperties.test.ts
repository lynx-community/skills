...
describe("Runtime.evaluate", () => {
  test("enables runtime and evaluates on the background VM by default", async () => {
    ...
    const { call } = createToolContext(Evaluate, connector as never, "test-client-id:9999");
    ...
    assert.deepStrictEqual(sentMessages, [
      { clientId: "test-client-id:9999", sessionId: 1, method: "Runtime.enable", params: {}, isMainThread: false },
      {
        ...
        params: { expression: "2 + 2", generatePreview: true, objectGroup: "mcp" },
        isMainThread: false,
      },
    ]);
  ...
  test("enables runtime and queries object properties on the requested VM thread", async () => {
    ...
    const connector = {
      sendCDPMessage: async (
        ...
        return { result: [{ name: "answer", value: { type: "number", value: 42 } }] };
      },
      sendListSessionMessage: async () => [{ session_id: 1 }],
    };

    const { call } = createToolContext(GetProperties, connector as never, "test-client-id:9999");
    ...
    assert.deepStrictEqual(result.result.map(({ name }) => name), ["answer"]);
  });
});
