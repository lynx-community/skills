...
async function loadDescribeNodeTool() {
  const module = await import('../../src/tools/DOM/DescribeNode.ts').catch(
    () => undefined,
  );

  assert.ok(
    module && 'DescribeNode' in module,
    'Expected DOM DescribeNode tool module to exist',
  );
...
describe('DOM.describeNode', () => {
  ...
  test('passes backendNodeId, depth, and pierce through to CDP', async () => {
    ...
    const connector = {
      sendCDPMessage: async (
        ...
        assert.strictEqual(method, 'DOM.describeNode');
        return {
          node: { backendNodeId: 9, childNodeCount: 1 },
          compress: false,
        };
      },
      sendListSessionMessage: async () => [{ session_id: 1 }],
    ...
    const result = await call<{
      node: { backendNodeId: number; childNodeCount: number };
      compress: boolean;
    }>({
      backendNodeId: 9,
      depth: -1,
  ...
  test('preserves Lynx depth semantics in returned nodes', async () => {
    ...
    const depthZero = await call<{
      node: { nodeId: number; childNodeCount: number; children?: unknown[] };
    }>({
      nodeId: 3,
      depth: 0,
    });
    const depthOne = await call<{
      node: { nodeId: number; children?: Array<{ children?: unknown[] }> };
    }>({
      nodeId: 3,
      depth: 1,
    });
