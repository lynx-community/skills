...
describe("App global switch tools", () => {
  ...
  test("App_getGlobalSwitch should read one key", async () => {
    ...
    const { call } = createToolContext(GetGlobalSwitch, mockConnector as never, testClientId);
    const result = await call<{ key: string; value: boolean }>({ key: "enable_devtool" });
  ...
  test("App_setGlobalSwitch should write one key", async () => {
    ...
    const mockConnector = createMockConnector({
      setGlobalSwitch: async (clientId: string, key: string, value: boolean) => {
        called = { clientId, key, value };
      },
    });

    const { call } = createToolContext(SetGlobalSwitch, mockConnector as never, testClientId);
    const result = await call<{ key: string; value: boolean }>({ key: "enable_devtool", switch: false });

    assert.deepStrictEqual(called, { clientId: testClientId, key: "enable_devtool", value: false });
    assert.deepStrictEqual(result, { key: "enable_devtool", value: false });
  });

  test("App_listGlobalSwitch should return all key states", async () => {
    const mockConnector = createMockConnector({
      getGlobalSwitch: async (_clientId: string, key: string) => key !== "enable_logbox",
    });

    const { call } = createToolContext(ListGlobalSwitch, mockConnector as never, testClientId);
    const result = await call<{ switches: Array<{ key: string; value?: boolean; error?: string }> }>({});
