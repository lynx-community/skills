...
describe("Device.openPage", () => {
  ...
  test("should send App.openPage message on success", async () => {
    ...
    const mockConnector = createMockConnector({
      sendAppMessage: async (cid: string, method: string, params: any) => {
        if (cid === testClientId) {
          sentMethod = method;
          sentParams = params;
        }
    ...
    const { call } = createToolContext(OpenPage, mockConnector as any, testClientId);
  ...
  test("should fallback to Customized event if App.openPage fails", async () => {
    ...
    const mockConnector = createMockConnector({
      ...
      sendMessage: async (cid: string, message: any, pipeline: any) => {
        sentMessage = message;
