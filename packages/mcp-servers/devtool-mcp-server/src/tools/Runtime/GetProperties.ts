...
export const GetProperties = /*#__PURE__*/ defineTool({
  ...
  schema: {
    ...
    sessionId,
    thread,
    objectId: z.string().describe("Remote object id returned by Runtime.evaluate or console output."),
    ownProperties: z.boolean().optional().describe("Return only properties owned by the object."),
    accessorPropertiesOnly: z.boolean().optional().describe("Return accessor properties only when supported."),
    generatePreview: z.boolean().optional().describe("Whether to generate previews for property values."),
    nonIndexedPropertiesOnly: z.boolean().optional().describe("Return non-indexed properties only when supported."),
  },
  annotations: {
    readOnlyHint: true,
  },
