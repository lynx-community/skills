...
export const GetScriptSource = /*#__PURE__*/ defineTool({
  ...
  schema: {
    ...
    saveToTmp: z.boolean()
      .optional()
      .describe("Whether to save the script source to a temporary file if it is large.")
      .default(false),
  },
  annotations: {
    readOnlyHint: true,
  ...
  async handler({ params }, response, context) {
    ...
    const { scriptSource } = await connector.sendCDPMessage<{ scriptSource: string }, { scriptId: string }>(
      params.clientId,
      params.sessionId,
