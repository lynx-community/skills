...
description: |
  ...
  - User needs a Lynx bundle with background, main-thread, and CSS assets, which must be build by rspeedy
  - User wants a Lynx-consumable JSON output which is generated by LLM, which can be consumed directly and do not need to bundle
---
...
1. If the user wants a Lynx-consumable JSON output, start with `references/lynx-json-render.md`. Otherwise, for bundle scenarios, start with `references/template-webpack-build.md` for package setup.
...
Prefer the smallest relevant examples:

- `examples/counter.md`: minimal app shape for scaffolding, exact file contents, or debugging the template-webpack bundle path; serves as a compact reference for both JSON and bundle outputs.
- `examples/todo-list.md`: composed app that combines first-screen data, patch-based rerendering, condition switching, repeat rendering, filters, and background event handling; serves both JSON and bundle outputs.
