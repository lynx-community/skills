...
import { getTestingSession, testWithClient } from "@lynx-js/devtool-connector/test-with-client";
import type { TestContext } from "node:test";
import { buildSubstringMatcher, findComponents } from "../src/commands/reactlynx/find.ts";
import { formatTree } from "../src/commands/reactlynx/format.ts";
import type { RendererState } from "../src/commands/reactlynx/protocol.ts";
import { buildOutboundFrame, type PreactEnvelope, runReactLynxSession } from "../src/commands/reactlynx/transport.ts";
...
run("reactlynx tree", async (t, connector, client, target) => {
  ...
  await t.test("init + refresh produces a non-empty tree", async (t: TestContext) => {
    ...
    t.assert.ok(formatted.text.length > 0, "Formatted tree must be non-empty");
    t.assert.ok(
    ...
    t.diagnostic(
      `frames=${collected.framesSeen} operation_v2=${collected.operationFrames} `
        + `root-order=${collected.rootOrderFrames} types=${
          [...collected.envelopeTypes].sort().join(",")
        } treeSize=${collected.state.tree.size}`,
    );
  ...
  await t.test("findComponents finds at least one match in the live tree", async (t: TestContext) => {
    if (!capturedTree) {
      t.skip("tree snapshot not captured (preceding subtest failed)");
      return;
    }
    const matches = findComponents(capturedTree, buildSubstringMatcher("Provider"), {
      hideShells: true,
      limit: 50,
    });
    t.assert.ok(
    ...
    for (const match of matches) {
      t.assert.match(match.label, /^@c\d+$/, `match.label must be @cN form, got ${match.label}`);
      t.assert.ok(match.name.toLowerCase().includes("provider"));
    }
    t.diagnostic(`find matches: ${matches.map((m) => `${m.label} ${m.name}`).join(", ")}`);
  });

  await t.test("inspect round-trip returns InspectData for first labelled component", async (t: TestContext) => {
    if (!capturedTree) {
      t.skip("tree snapshot not captured (preceding subtest failed)");
      return;
    }
    ...
    t.assert.ok(targetId !== undefined, "tree must expose at least one labelled root");
    ...
    t.assert.equal(inspectData.id, targetId, "inspect-result.id must match the requested id");
    t.assert.equal(typeof inspectData.name, "string");
    t.assert.ok(inspectData.name.length > 0, "inspect-result.name must be non-empty");
    t.diagnostic(`inspect-result: id=${inspectData.id} name=${inspectData.name}`);
  });

  await t.test("update-prop round-trip applies the new value and confirms via inspect-result", async (t: TestContext) => {
    if (!capturedTree) {
      t.skip("tree snapshot not captured (preceding subtest failed)");
      return;
    }
    ...
    await runReactLynxSession({
      ...
      onEnvelope: (envelope: PreactEnvelope) => {
        if (envelope.type !== "inspect-result") return "continue";
        const candidate = envelope.data as { id?: number; props?: Record<string, unknown> };
