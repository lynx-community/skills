...
export async function getLatestSession(connector: Connector, clientId: string): Promise<string> {
...
export async function resolveClient(
  ...
  const clientId = options.client
    ?? (options.clientName ? await getClientByName(connector, options.clientName) : await getFirstClient(connector));
...
export function buildWatchSignal(
  ...
  if (!watch) {
    return { signal: AbortSignal.timeout(fallbackTimeoutMs), cleanup: () => {} };
  }
...
async function readOrTimeout<T>(
  ...
  const idle = delay(idleMs, "timeout" as const, { signal: idleAbortController.signal });
