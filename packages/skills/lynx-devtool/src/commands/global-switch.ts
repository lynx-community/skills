...
import { CLIENT_NAME_OPTION, CLIENT_OPTION, type Context, parseOnOff, resolveClient } from "./utils.ts";
...
export function registerGlobalSwitchCommand(program: Command, context: Context) {
  ...
  globalSwitch
    ...
    .action(async (options) => {
      ...
      const switches: Array<{ key: GlobalSwitchKey; value?: boolean; error?: string }> = [];
  ...
  globalSwitch
    ...
    .requiredOption("--key <key>", `Global switch key. Supported: ${GLOBAL_SWITCH_KEYS_HELP}`)
    .option(...CLIENT_OPTION)
    .option(...CLIENT_NAME_OPTION)
    .action(async (options) => {
      const { connector, clientId } = await resolveClient(context, options);
