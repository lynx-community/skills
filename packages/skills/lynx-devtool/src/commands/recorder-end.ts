...
import { analyzeRecordingBuffer, type FileDiagnostic, recordingOutputPath } from "./recorder-analysis.ts";
import { CLIENT_NAME_OPTION, CLIENT_OPTION, type Context, isAbortError, resolveClient } from "./utils.ts";
...
export function registerEndCommand(parent: Command, context: Context) {
  parent
    ...
    .action(async (options) => {
      ...
      console.log(JSON.stringify({ success: true, message: "Recording ended successfully.", ...result }));
    });
}
...
export async function runRecordingEnd(
...
): Promise<{ savedFiles: string[]; recordingComplete: Record<string, unknown>; diagnostics: FileDiagnostic[] }> {
  ...
  if (savedFiles.length === 0) {
    throw new Error(
      buildNoPageRecordingMessage(clientId, recordingComplete, streams, sessionIDs),
    );
  }
...
function buildNoPageRecordingMessage(
  ...
  const nativeFiles = Array.isArray(filenames) && filenames.length > 0
...
async function readRecordingCompleteEvent(
  ...
  const isTimeoutError = (err: unknown) => isAbortError(err) || (err instanceof Error && err.name === "TimeoutError");
  ...
  try {
    ...
    for await (
      const value of stream as unknown as AsyncIterable<
        CDPResponseMessage & {
          method?: string;
...
async function resolveRecordingBaseOutputPath(output: string | undefined, clientId: string): Promise<string> {
  const defaultFileName = `recording-${clientId.replace(/[<>:"/\\|?*()]/g, "_")}-${Date.now()}.json`;
  if (!output) {
    return path.resolve(os.homedir(), ".lynx-devtool", "files", "lynxrecorder", defaultFileName);
  }

  const resolvedOutput = path.resolve(output);
  const outputLooksLikeDirectory = output.endsWith(path.sep) || output.endsWith("/") || output.endsWith("\\");
  const outputIsDirectory = outputLooksLikeDirectory
    || await fs.stat(resolvedOutput).then(stats => stats.isDirectory()).catch(() => false);
