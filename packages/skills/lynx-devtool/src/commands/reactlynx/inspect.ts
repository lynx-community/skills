...
export function formatInspectResult(data: InspectResult, ref: string): string {
  ...
  if (data.suspended !== undefined && data.suspended) lines.push("  suspended: true");
    ...
    .action(async (ref: string, options: ComponentOptions) => {
      ...
      const inspectSession = await runReactLynxSession({
        ...
        onEnvelope: (env: PreactEnvelope) => {
          if (env.type === "inspect-result" && env.data && typeof env.data === "object") {
      ...
      if (!inspectResult) {
        const types = [...inspectSession.envelopeTypes].sort().join(",") || "(none)";
        process.stderr.write(
