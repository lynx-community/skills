...
export function registerUpdateCommands(
  ...
  registerOneUpdate(reactlynx, context, {
    name: 'update-prop',
    description:
      'Set a prop on a single ReactLynx component (forceUpdate is called for you)',
    kind: 'update-prop',
  });
  registerOneUpdate(reactlynx, context, {
    name: 'update-state',
    description:
      'Set a state field on a single class component (forceUpdate is called for you)',
    kind: 'update-state',
  });
  registerOneUpdate(reactlynx, context, {
    name: 'update-context',
    ...
    .action(
      async (
        ...
        const { connector, clientId, sessionId } =
          await resolveClientAndSession(context, options);
