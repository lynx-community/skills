...
function formatConsoleMessage({ type, args, stackTrace, consoleTag }: ConsoleMessage): string {
  return `- [${type}/${consoleTag === "Lepus" ? "main-thread" : "background"}]: ${
    args.map((arg) => {
      if (arg.objectId) {
...
export function registerGetConsoleCommand(program: Command, context: Context) {
  program
    ...
    .option("--thread <thread...>", "VM thread to target: background or main", ["background", "main"])
    .option(
      "-w, --watch",
    ...
    .action(async (options) => {
      ...
      for await (const value of readUntilIdle(stream, { idleMs: 500, maxMs: 5000 })) {
        if (value.method !== "Runtime.consoleAPICalled") continue;
        const params = value.params as ConsoleMessage;
        if (!allowedLevels.includes(params.type)) continue;
