...
export function analyzeRecordingBuffer(filePath: string, buffer: Buffer): FileDiagnostic {
  ...
  const hasTemplate = actions.some(a => a["Function Name"] === "loadTemplate");
  const hasTouchEvents = actions.some(a =>
    a["Function Name"] === "SendTouchEvent" || a["Function Name"] === "sendEventDarwin"
  );
...
export function recordingOutputPath(basePath: string, sessionId: number, index: number): string {
